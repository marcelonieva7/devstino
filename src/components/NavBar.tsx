  ...
  return (
    <nav>
      <ul className="text-zinc-300 flex justify-between gap-4 px-4 py-5 after:bg-zinc-800 underline-offset-4">
        <Link className={clsx("text-xl hover:underline hover:text-red-700 transition-colors self-start", {"underline text-red-700": pathname === ROUTES.HOME})} href={ROUTES.HOME}>{TITLES[0]}</Link>          
        <Link
          className={clsx("text-xl hover:underline hover:text-red-700 transition-colors self-start", {
            ...
            "underline text-red-700": currentPath === path.dirname(ROUTES.ADD.STEP_ONE) || pathname === devstino
          })}
